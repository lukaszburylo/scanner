...
class PingScanner(AbstractScanner):
    ...
    def scan(self):
        try:
            subprocess.check_output(["ping", "-c", "1", "-W", "1", "-I", self.interface, self.ip], stderr=subprocess.PIPE)
            return True
...
class ArpingScanner(AbstractScanner):
    ...
    def scan(self):
        try:
            subprocess.check_output(["arping", "-c", "1", "-w", "1", "-I", self.interface, self.ip], stderr=subprocess.PIPE)
            return True
        except subprocess.CalledProcessError:
            return False
